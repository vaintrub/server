...
dberr_t recv_sys_t::find_checkpoint()
{
  ...
  upgrade:
    ...
    recovered_lsn= mlog_checkpoint_lsn= log_sys.next_checkpoint_lsn;
  ...
  same_page:
    ...
    else if (rlen)
    {
      ...
      case FILE_CHECKPOINT:
        ...
        {
          if (UNIV_LIKELY(lsn >= log_sys.log.get_first_lsn()))
          {
...
static bool recv_scan_log(bool last_phase)
{
  ...
  ut_ad(!last_phase || recv_sys.mlog_checkpoint_lsn);

  store_t store= last_phase
    ? STORE_IF_EXISTS : recv_sys.mlog_checkpoint_lsn ? STORE_YES : STORE_NO;
...
read_only_recovery:
		...
		if (recv_sys.recovered_lsn
		    != log_sys.next_checkpoint_lsn + sizeof_checkpoint) {
			recv_sys.recovered_lsn = log_sys.next_checkpoint_lsn;
			recv_sys.recovered_offset = 0;
			recv_sys.len = 0;
		}
